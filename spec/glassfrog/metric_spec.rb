...
describe Glassfrog::Metric do
  ...
  describe '#get' do
    ...
    it 'should return an array of metric item objects with singular symbol as type' do
      array_of_metrics = @client.get :metric
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end
    it 'should return an array of metric item objects with plural symbol as type' do
      array_of_metrics = @client.get :metrics
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end
    it 'should return an array of metric item objects with singular string as type' do
      array_of_metrics = @client.get 'metric'
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end
    it 'should return an array of metric item objects with plural string as type' do
      array_of_metrics = @client.get 'metrics'
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end

    it 'should return an array of metric item objects with string as options' do
      array_of_metrics = @client.get :metric, "#{@metric.id}"
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end
    it 'should return an array of metric item objects with integer as options' do
      array_of_metrics = @client.get :metric, @metric.id
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end

    it 'should return an array of metric item objects with id in hash as options' do
      array_of_metrics = @client.get :metric, { id: @metric.id }
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end
    it 'should return an array of metric item objects with metric item object as options' do
      array_of_metrics = @client.get :metric, @metric
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end

    it 'should return an array of metric item objects with circle object as options' do
      array_of_metrics = @client.get :metric, @circle
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end
    it 'should return an array of metric item objects with circle_id in hash as options' do
      array_of_metrics = @client.get :metric, { circle_id: @circle.id }
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end

    it 'should return an array of metric item objects with role object as options' do
      array_of_metrics = @client.get :metric, @role
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end
    it 'should return an array of metric item objects with role_id in hash as options' do
  ...
  describe '#post' do
    ...
    it 'should create a new metric object on GlassFrog with hash as options and returns this new object' do
      array_of_metrics = @client.post :metric, @new_metric_hash
      expect(array_of_metrics).to all(be_a(Glassfrog::Metric))
    end
    it 'should create a new metric object on GlassFrog with a metric object as options and returns this new object' do
  ...
  describe '#patch' do
    ...
    it 'should update a metric object on GlassFrog with a metric object as options without identifier' do
    ...
    it 'should update a metric object on GlassFrog with a hash as options without identifier' do
      options = @client.patch :metric, @metric_hash
      expect(options).not_to be(false)
      expect(@client.get(:metric, options).first.description).to eq(@metric_hash[:description])
    end
    it 'should update a metric object on GlassFrog with a metric object as options with identifier' do
      id = @metric.id
      options = @client.patch :metric, id, @metric
      expect(options).not_to be(false)
      expect(@client.get(:metric, options).first.description).to eq(@metric.description)
    end
    it 'should update a metric object on GlassFrog with a hash as options with identifier' do
      id = @metric_hash[:id]
  ...
  describe '#delete' do
    ...
    it 'should delete a metric on GlassFrog with a metric as options' do
      @metric = @client.post(:metric, @new_metric_hash).first
      expect(@client.delete :metric, @metric).to be(true)
    end

    it 'should delete a metric on GlassFrog with a hash as options' do
      @metric = @client.post(:metric, @new_metric_hash).first
